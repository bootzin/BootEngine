...
{
	public sealed class Render2DSystem : IEcsRunSystem
	{
		private readonly EcsFilter<TransformComponent, SpriteRendererComponent> QuadsFilter = default;
			...
			foreach (int camera in CameraFilter)
			{
				...
				if (cam.Camera.Active)
				{
					...
					foreach (int quad in QuadsFilter)
					{
						...
						// Filter quads within camera fustrum?
						Renderer2D.Instance.QueueQuad(transform.Translation, transform.Scale, transform.Rotation, sprite.Color, sprite.SpriteData, sprite.Material);
					}
